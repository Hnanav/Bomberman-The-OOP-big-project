...
import static bomberman.bomberman.Algorithm.SetUpLevel.*;
...
public class RandMap {
    ...
    public static void SetUpWall() {
        ...
        for (Entity get : Enemies)
            if (map[(int) get.getPos().getX()][(int) get.getPos().getY()] == 'E') {
            int i = twoForOne((int) get.getPos().getX(), (int) get.getPos().getY());
            maze1D[i] = 1;
            cnt += 1;
        }
        while (block < Wall) {
    ...
    public static boolean check(int x, int y) {
        ...
        for (Entity get : Enemies) {
            int cnt = 0;
            for (int i = Math.max(2, (int) get.getPos().getX() - 4);
                 i < Math.min((int) get.getPos().getX() + 4, 19); ++ i)
                for (int j = Math.max(2, (int) get.getPos().getY() - 4);
                     j < Math.min((int) get.getPos().getY() + 4, 19); ++ j)
                            if (map[i][j] == ' ') cnt ++;
            int num = 0;
            if ((int)get.getPos().getX() > 0)
                if (map[(int)get.getPos().getX() - 1][(int)get.getPos().getY()] == ' ') num ++;
            if (get.getPos().getX() < 19)
                if (map[(int)get.getPos().getX() + 1][(int)get.getPos().getY()] == ' ') num ++;
            if (get.getPos().getY() > 0)
                if (map[(int)get.getPos().getX()][(int)get.getPos().getY() - 1] == ' ') num ++;
            if (get.getPos().getY() < 19)
                if (map[(int)get.getPos().getX()][(int)get.getPos().getY() + 1] == ' ') num ++;
            if (num == 0 || cnt < 8) {
                map[x][y] = ' ';
                return false;
    ...
    public static boolean checkDistance(int x, int y) {
        ...
        int Pycan = 1;
        for (Entity get : Enemies) {
            double dis = ((int) get.getPos().getX() - x) * ((int) get.getPos().getX() - x) +
                    (get.getPos().getY() - y) * (get.getPos().getY() - y);
            long distance = Math.round(Math.sqrt(dis));
            if ((int) get.getPos().getX() == x) Pxcan ++;
            if (get.getPos().getY() == y) Pycan ++;
            if (can > distance) return false;
        }
        return Pxcan < xycan && Pycan < xycan;
    }
    ...
    public static boolean check1() {
        for (Entity get : Enemies) {
            int cnt = 0;
            if (map[(int) get.getPos().getX()][(int) get.getPos().getY()] == 'E') {
                if (map[(int) get.getPos().getX() + 1][(int) get.getPos().getY()] != ' ') cnt ++;
                if (map[(int) get.getPos().getX()][(int) get.getPos().getY() + 1] != ' ') cnt ++;
                if (map[(int) get.getPos().getX() - 1][(int) get.getPos().getY()] != ' ') cnt ++;
                if (map[(int) get.getPos().getX()][(int) get.getPos().getY() - 1] != ' ') cnt ++;
            }
