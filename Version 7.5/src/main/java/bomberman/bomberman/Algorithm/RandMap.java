...
public class RandMap {
    ...
    public static boolean check(int x, int y) {
        ...
        for (Entity get : Enemies) {
            int cnt = 0;
            for (int i = Math.max(2, (int) get.getPos().getX() - 4); i < Math.min((int) get.getPos().getX() + 4, 19); ++ i)
            for (int j = Math.max(2, (int) get.getPos().getY() - 4); j < Math.min((int) get.getPos().getY() + 4, 19); ++ j)
                if (map[i][j] == ' ') cnt ++;
            if (cnt < 8) {
                map[x][y] = ' ';
                return false;
